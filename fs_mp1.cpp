...
void lfs_rm(String filename){
	...
	dir.file_name.erase(order);
	dir.inode.erase(order);
	dir.file_type.erase(order);

	inode_table.inode.erase(order-1);
	inode_table.ref_cnt.erase(order-1);
	inode_table.file_size.erase(order-1);
	inode_table.data_block.erase(order-1);
	inode_table.file_type.erase(order-1);

	db.content.erase(order-2);
	db.data_block.erase(order-2);
...
void cat(String filename){
	...
	for(int i=0; i<db.content.size();i++){
		if(filename.compare(dir.file_name[i])==0){
			order=i;
			break;
		}
	...
	outf<<db.content[order-2]<<endl;


}

void lfs_mkdir(String dirname){
	char a[100];
	my_itoa(data_block, a);
	dir.file_name.push_back(filename);
	dir.file_type.push_back('-');
